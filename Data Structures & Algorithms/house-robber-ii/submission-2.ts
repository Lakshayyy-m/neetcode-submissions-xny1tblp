class Solution {
    ...
    rob(nums: number[]): number {
        ...
        const dfs = (index: number, isFirstValue: boolean = false) => {
            if (memo[index][isFirstValue.toString()]) return memo[index][isFirstValue.toString()];
            let valueToAdd = 0;
            ...
            memo[index][isFirstValue.toString()] = nums[index] + valueToAdd;
            return memo[index][isFirstValue.toString()];
        };
