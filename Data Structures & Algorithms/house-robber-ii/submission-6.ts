class Solution {
    ...
    rob(nums: number[]): number {
        ...
        const memo = Array.from({ length: nums.length }, () => Array(2).fill(null));

        const dfs = (index: number, isFirstValue: number = 0) => {
            if (memo[index][isFirstValue]) return memo[index][isFirstValue];
            let valueToAdd = 0;
            ...
            memo[index][isFirstValue] = nums[index] + valueToAdd;
            return memo[index][isFirstValue];
        };

        return Math.max(dfs(0, 1), dfs(1), dfs(2));
    }
}
